const test = document.getElementById('paintings')
...
function jsononscreen(data) {
    ...
    data.forEach(element => {
        ...
        test.innerHTML += "<div class='artcards'> " + "<img data-index=" + count + " src='" + element.path + "' alt='' style='width: 300px;' />"
            + "<li>" + "<span>" + "TITLE: " + "</span>" + element.title + "</li>" + "<li>" + "<span>" + "YEAR: " + "</span>" + element.date + "</li>"
            + "<li>" + "<span>" + "PRICE: " + "</span>" + element.price + "</li>" + "</div>";
        count++
    });
...
function showDetail(event) {
    ...
    fetch('paintings.json')
        ...
        .then(data => {
            ...
            const containerDiv = document.getElementById('paintings');
            containerDiv.classList.add('singlecontainer');

            // const backButton = document.createElement('button');
            // backButton.textContent = 'Back';
            // backButton.addEventListener('click', () => {
            //     containerDiv.classList.remove('singlecontainer');
